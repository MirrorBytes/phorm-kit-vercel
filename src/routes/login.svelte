<script lang="ts">
	...
	import type Login from '../entities/validators/login';
	...
	const submit = async (e: CustomEvent<SubmitType>) => {
		...
		const response = await fetch('/graphql', {
			body: JSON.stringify({
				query: `mutation LoginUser($data: Login!) {
					login(data: $data) {
						ok
						msg
					}
				...
				variables: {
					data: (get(store) as unknown) as Login
				}
			}),
			headers: {
		...
		if (data) {
			if (data.login.ok) {
				success(data.login.msg);

				authTrigger.update((v) => !v);

				goto('/');
			} else {
				danger(data.login.msg);
			}
		} // TODO: Check errors.
	};
</script>
